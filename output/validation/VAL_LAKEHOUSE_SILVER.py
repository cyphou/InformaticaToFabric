...
source_table = "SCHEMA.Oracle_CRM"

key_columns = ['lakehouse_silver_id']
...
try:
    ...
    distinct_keys = df_target.select("lakehouse_silver_id").distinct().count()
    key_status = "PASS" if total == distinct_keys else "FAIL"
    dq_checks.append({
        "rule": "Key Uniqueness: lakehouse_silver_id",
        "result": key_status,
