...
key_columns = ['tgt_bronze_cdc_events_id']
...
try:
    ...
    distinct_keys = df_target.select("tgt_bronze_cdc_events_id").distinct().count()
    key_status = "PASS" if total == distinct_keys else "FAIL"
    dq_checks.append({
        "rule": "Key Uniqueness: tgt_bronze_cdc_events_id",
        "result": key_status,
