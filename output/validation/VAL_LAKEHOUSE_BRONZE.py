...
# Source: S3_LANDING → Target: main.silver.lakehouse_bronze
# Mapping: MI_BULK_LOAD_PRODUCTS
# =============================================================================
...
mapping_name = "MI_BULK_LOAD_PRODUCTS"
target_table = "main.silver.lakehouse_bronze"
...
source_table = "SCHEMA.S3_LANDING"

key_columns = ['lakehouse_bronze_id']
...
try:
    ...
    distinct_keys = df_target.select("lakehouse_bronze_id").distinct().count()
    key_status = "PASS" if total == distinct_keys else "FAIL"
    dq_checks.append({
        "rule": "Key Uniqueness: lakehouse_bronze_id",
        "result": key_status,
