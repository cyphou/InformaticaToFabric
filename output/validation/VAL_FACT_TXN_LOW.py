...
# =============================================================================
# Validation Notebook: VAL_FACT_TXN_LOW
# Source: Oracle.FINANCE.TRANSACTIONS, Oracle.FINANCE.ACCOUNTS → Target: main.silver.fact_txn_low
...
target_table = "main.silver.fact_txn_low"
...
try:
    ...
    distinct_keys = df_target.select("txn_low_id").distinct().count()
    key_status = "PASS" if total == distinct_keys else "FAIL"
    dq_checks.append({
        "rule": "Key Uniqueness: txn_low_id",
        "result": key_status,
