...
# =============================================================================
# Validation Notebook: VAL_FACT_TXN_HIGH
# Source: Oracle.FINANCE.TRANSACTIONS, Oracle.FINANCE.ACCOUNTS → Target: main.silver.fact_txn_high
...
target_table = "main.silver.fact_txn_high"
...
try:
    ...
    distinct_keys = df_target.select("txn_high_id").distinct().count()
    key_status = "PASS" if total == distinct_keys else "FAIL"
    dq_checks.append({
        "rule": "Key Uniqueness: txn_high_id",
        "result": key_status,
